...
def test_transitions_and_track_isolation():
    ...
    assert machine.update(1, "SW", 2) == State.INSIDE_MOVING
    assert machine.update(1, "SW", 4, True, 3) == State.STATIONARY
    assert machine.update(1, "SW", 5) == State.INSIDE_MOVING
    assert machine.update(2, "SW", 5) == State.ENTERING
    assert machine.update(1, "SW", 40, True, 30) == State.SUSPECTED_VIOLATION
